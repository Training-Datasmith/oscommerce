...
class Template
{
    ...
    protected $_page_tags = ['generator' => ['osCommerce Online Merchant']];
    ...
    protected $_javascript_filenames = ['public/sites/Shop/javascript/general.js'];
    ...
    protected $_javascript_php_filenames = [];
        ...
        if (!isset($_SESSION['template']) || !empty($code) || (isset($_GET['template']) && !empty($_GET['template']))) {
            ...
            foreach ($this->getTemplates() as $template) {
                if ($template['code'] == DEFAULT_TEMPLATE) {
                    $data_default = ['id' => $template['id'], 'code' => $template['code']];
    ...
    public function _getJavascriptFilenames()
    {
        $js_files = '';
    ...
    public function _getJavascriptPhpFilenames()
    {
    ...
    public function _getJavascriptBlocks()
    {
