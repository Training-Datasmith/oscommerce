...
abstract class ApplicationAbstract
{
    ...
    public function runActions()
    {
        ...
        if (isset($action)) {
            call_user_func(['osCommerce\\OM\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\Action\\' . $action, 'execute'], $this);
            ...
            if ($action_index < count($_GET)) {
                ...
                for ($i = $action_index, $n = count($_GET); $i < $n; $i++) {
                    ...
                    if (!in_array($subaction, $this->_ignored_actions) && self::siteApplicationActionExists(implode('\\', $action) . '\\' . $subaction)) {
                        call_user_func(['osCommerce\\OM\\Core\\Site\\' . OSCOM::getSite() . '\\Application\\' . OSCOM::getSiteApplication() . '\\Action\\' . implode('\\', $action) . '\\' . $subaction, 'execute'], $this);
