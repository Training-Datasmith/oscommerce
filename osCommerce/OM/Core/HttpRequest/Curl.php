...
class Curl
{
    public static function execute($parameters)
    {
        ...
        $curl_options = [CURLOPT_PORT => $parameters['server']['port'], CURLOPT_HEADER => true, CURLOPT_SSL_VERIFYPEER => true, CURLOPT_SSL_VERIFYHOST => 2, CURLOPT_RETURNTRANSFER => true, CURLOPT_FORBID_REUSE => true, CURLOPT_FRESH_CONNECT => true, CURLOPT_FOLLOWLOCATION => false];
        ...
        if ($http_code == 301 || $http_code == 302) {
            if (!isset($parameters['redir_counter']) || $parameters['redir_counter'] < 6) {
                ...
                $body = \Os_Commerce\OM\Core\Http_Request::get_response($redir_params, 'Curl');
            }
        }
        return $body;
    }
