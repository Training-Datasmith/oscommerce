...
while ($Qboxes->next()) {
    $boxes_array[] = ['id' => $Qboxes->valueInt('id'),
                           'text' => $Qboxes->value('title')];
}

$pages_array = [['id' => $filter_id . '/*',
                           'text' => '*']];
...
foreach ($d_boxes->getFiles(false) as $box) {
    if ($box['is_directory'] === true) {
        $entry = ['id' => $filter_id . '/' . $box['name'] . '/*',
                       'text' => $box['name'] . '/*'];
    } else {
        $page_filename = substr($box['name'], 0, strrpos($box['name'], '.'));

        $entry = ['id' => $filter_id . '/' . $page_filename,
                       'text' => $page_filename];
    }
...
while ($Qgroups->next()) {
    $groups_array[] = ['id' => $Qgroups->value('boxes_group'),
                            'text' => $Qgroups->value('boxes_group')];
}

if (!empty($groups_array)) {
    array_unshift($groups_array, ['id' => null, 'text' => $osC_Language->get('please_select')]);
}
