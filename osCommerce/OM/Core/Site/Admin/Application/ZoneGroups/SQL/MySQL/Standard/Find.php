...
class Find
{
    public static function execute($data)
    {
        ...
        $Qgroups->bind_value(':geo_zone_name', '%' . $data['keywords'] . '%');
        $Qgroups->bind_value(':geo_zone_description', '%' . $data['keywords'] . '%');
        $Qgroups->bind_value(':countries_name', '%' . $data['keywords'] . '%');
        $Qgroups->bind_value(':zone_name', '%' . $data['keywords'] . '%');
        if ($data['batch_pageset'] !== -1) {
