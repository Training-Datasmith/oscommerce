...
class Get_Js_List
{
    public static function execute($country, $form, $field)
    {
        ...
        while ($Qcountries->fetch()) {
            ...
            $Qzones->bind_int(':zone_country_id', $Qcountries->value_int('zone_country_id'));
            $Qzones->execute();
            while ($Qzones->fetch()) {
                if ($num_state == '1') {
                    $output_string .= '    ' . $form . '.' . $field . '.options[0] = new Option("' . OSCOM::get_def('all_zones') . '", "");' . "\n";
                }
                $output_string .= '    ' . $form . '.' . $field . '.options[' . $num_state . '] = new Option("' . $Qzones->value('zone_name') . '", "' . $Qzones->value_int('zone_id') . '");' . "\n";
                $num_state++;
            }
            $num_country++;
        }
        $output_string .= '  } else {' . "\n" . '    ' . $form . '.' . $field . '.options[0] = new Option("' . OSCOM::get_def('all_zones') . '", "");' . "\n" . '  }' . "\n";
