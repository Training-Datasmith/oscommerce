...
<?php 
$Qstatuses = $os_c_database->query('select orders_status_id, orders_status_name from :table_orders_status where language_id = :language_id order by orders_status_name');
$Qstatuses->bind_table(':table_orders_status', TABLE_ORDERS_STATUS);
$Qstatuses->bind_int(':language_id', $os_c_language->get_id());
$Qstatuses->set_batch_limit($_GET['page'], MAX_DISPLAY_SEARCH_RESULTS);
$Qstatuses->execute();
?>
...
<table border="0" width="100%" cellspacing="0" cellpadding="2">
  <tr>
    <td><?php 
echo $Qstatuses->get_batch_total_pages($os_c_language->get('batch_results_number_of_entries'));
?></td>
    <td align="right"><?php 
echo $Qstatuses->get_batch_page_links('page', $os_c_template->get_module(), false);
?></td>
  </tr>
</table>
...
<table border="0" width="100%" cellspacing="0" cellpadding="2" class="dataTable">
  <thead>
    <tr>
      <th><?php 
echo $os_c_language->get('table_heading_order_statuses');
?></th>
      <th width="150"><?php 
...
while ($Qstatuses->next()) {
    ...
    if ($Qstatuses->value_int('orders_status_id') == DEFAULT_ORDERS_STATUS_ID) {
        $status_name .= ' (' . $os_c_language->get('default_entry') . ')';
    }
    ?>

    <tr onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);">
      <td onclick="document.getElementById('batch<?php 
    echo $Qstatuses->value_int('orders_status_id');
...
<?php 
    echo osc_link_object(osc_href_link_admin(FILENAME_DEFAULT, $os_c_template->get_module() . '&page=' . $_GET['page'] . '&osID=' . $Qstatuses->value_int('orders_status_id') . '&action=save'), osc_icon('edit.png')) . '&nbsp;' . osc_link_object(osc_href_link_admin(FILENAME_DEFAULT, $os_c_template->get_module() . '&page=' . $_GET['page'] . '&osID=' . $Qstatuses->value_int('orders_status_id') . '&action=delete'), osc_icon('trash.png'));
    ?>

      </td>
      <td align="center"><?php 
    echo osc_draw_checkbox_field('batch[]', $Qstatuses->value_int('orders_status_id'), null, 'id="batch' . $Qstatuses->value_int('orders_status_id') . '"');
    ?></td>
    </tr>
...
?></td>
    <td align="right"><?php 
echo $Qstatuses->get_batch_pages_pull_down_menu('page', $os_c_template->get_module());
?></td>
  </tr>
</table>
