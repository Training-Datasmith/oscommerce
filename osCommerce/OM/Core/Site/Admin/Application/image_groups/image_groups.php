...
class Os_C_application_image_groups extends Os_C_template_admin
{
    /* Protected variables */
            ...
            switch ($_GET['action']) {
                case 'save':
                    ...
                    if (isset($_POST['subaction']) && $_POST['subaction'] == 'confirm') {
                        $data = ['title' => $_POST['title'], 'code' => $_POST['code'], 'width' => $_POST['width'], 'height' => $_POST['height'], 'force_size' => isset($_POST['force_size']) && $_POST['force_size'] == 'on' ? true : false];
                        if (Os_C_image_Groups_admin::save(isset($_GET['gID']) ? $_GET['gID'] : null, $data, isset($_POST['default']) && $_POST['default'] == 'on' ? true : false)) {
                ...
                case 'delete':
                    ...
                    if (isset($_POST['subaction']) && $_POST['subaction'] == 'confirm') {
                        if (Os_C_image_Groups_admin::delete($_GET['gID'])) {
                            $os_c_message_stack->add($this->_module, $os_c_language->get('ms_success_action_performed'), 'success');
                        } else {
                            $os_c_message_stack->add($this->_module, $os_c_language->get('ms_error_action_not_performed'), 'error');
                        }
                    ...
                    if (isset($_POST['batch']) && is_array($_POST['batch']) && !empty($_POST['batch'])) {
                        ...
                        if (isset($_POST['subaction']) && $_POST['subaction'] == 'confirm') {
                            ...
                            foreach ($_POST['batch'] as $id) {
                                if (!Os_C_image_Groups_admin::delete($id)) {
                                    $error = true;
                                    break;
                                }
                            }
