...
echo HTML::button(['href' => OSCOM::get_link(null, null, 'id=' . $_GET['id']), 'priority' => 'secondary', 'icon' => 'triangle-1-w', 'title' => OSCOM::get_def('button_back')]) . ' ' . HTML::button(['href' => OSCOM::get_link(null, null, 'InsertDefinition&id=' . $_GET['id'] . '&group=' . $_GET['group']), 'icon' => 'plus', 'title' => OSCOM::get_def('button_insert')]);
?></span>
</form>
...
<table border="0" width="100%" cellspacing="0" cellpadding="2" class="dataTable" id="defsDataTable">
  <thead>
    <tr>
      <th><?php 
echo OSCOM::get_def('table_heading_key');
?></th>
      <th><?php 
echo OSCOM::get_def('table_heading_value');
?></th>
      <th width="150"><?php 
...
echo HTML::submit_image(HTML::icon_raw('edit.png'), OSCOM::get_def('icon_edit'), 'onclick="document.batch.action=\'' . OSCOM::get_link(null, null, 'BatchSaveDefinitions&id=' . $_GET['id'] . '&group=' . $_GET['group']) . '\';"') . '&nbsp;' . HTML::submit_image(HTML::icon_raw('trash.png'), OSCOM::get_def('icon_trash'), 'onclick="document.batch.action=\'' . OSCOM::get_link(null, null, 'BatchDeleteDefinitions&id=' . $_GET['id'] . '&group=' . $_GET['group']) . '\';"');
?></th>
      <th align="center" width="20"><?php 
echo HTML::checkbox_field('batchFlag', null, null, 'onclick="flagCheckboxes(this);"');
?></th>
...
echo OSCOM::get_rpc_link(null, null, 'GetDefinitions&id=' . $_GET['id'] . '&group=' . $_GET['group']);
?>';

  var defEditLink = '<?php 
echo OSCOM::get_link(null, null, 'EditDefinition&id=' . $_GET['id'] . '&group=' . $_GET['group'] . '&dID=DEFINITIONID');
?>';
...
echo OSCOM::get_link(null, null, 'DeleteDefinition&id=' . $_GET['id'] . '&group=' . $_GET['group'] . '&dID=DEFINITIONID');
?>';
  var defDeleteLinkIcon = '<?php 
