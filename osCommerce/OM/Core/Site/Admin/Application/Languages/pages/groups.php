...
echo HTML::button(['href' => OSCOM::get_link(), 'priority' => 'secondary', 'icon' => 'triangle-1-w', 'title' => OSCOM::get_def('button_back')]) . ' ' . HTML::button(['href' => OSCOM::get_link(null, null, 'InsertDefinition&id=' . $_GET['id']), 'icon' => 'plus', 'title' => OSCOM::get_def('button_insert')]);
?></span>
</form>
...
<table border="0" width="100%" cellspacing="0" cellpadding="2" class="dataTable" id="langDefGroupDataTable">
  <thead>
    <tr>
      <th><?php 
echo OSCOM::get_def('table_heading_definition_groups');
?></th>
      <th width="150"><?php 
...
echo '<b>' . OSCOM::get_def('table_action_legend') . '</b> ' . HTML::icon('trash.png') . '&nbsp;' . OSCOM::get_def('icon_trash');
?></span>
...
echo OSCOM::get_rpc_link(null, null, 'GetGroups&id=' . $_GET['id']);
?>';

  var groupLink = '<?php 
echo OSCOM::get_link(null, null, 'id=' . $_GET['id'] . '&group=GROUPCODE');
?>';
...
echo OSCOM::get_link(null, null, 'DeleteGroup&id=' . $_GET['id'] . '&group=GROUPCODE');
?>';
  var groupDeleteLinkIcon = '<?php 
