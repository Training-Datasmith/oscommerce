...
foreach (ObjectInfo::to(Languages::getGroups($OSCOM_ObjectInfo->getInt('languages_id')))->get('entries') as $group) {
    $groups_array[] = ['id' => $group['content_group'],
...
<div class="infoBox">
  ...
  <fieldset>
    <p>(<a href="javascript:selectAllFromPullDownMenu('groups');"><u><?php echo OSCOM::getDef('select_all'); ?></u></a> | <a href="javascript:resetPullDownMenuSelection('groups');"><u><?php echo OSCOM::getDef('select_none'); ?></u></a>)<br /><?php echo HTML::selectMenu('groups[]', $groups_array, ['account', 'checkout', 'general', 'index', 'info', 'order', 'products', 'search'], 'id="groups" size="10" multiple="multiple"'); ?></p>

    <p><?php echo HTML::checkboxField('include_data', [['id' => '', 'text' => OSCOM::getDef('field_export_with_data')]], true); ?></p>
  </fieldset>

  <p><?php echo HTML::button(['priority' => 'primary', 'icon' => 'triangle-1-nw', 'title' => OSCOM::getDef('button_export')]) . ' ' . HTML::button(['href' => OSCOM::getLink(), 'priority' => 'secondary', 'icon' => 'close', 'title' => OSCOM::getDef('button_cancel')]); ?></p>
