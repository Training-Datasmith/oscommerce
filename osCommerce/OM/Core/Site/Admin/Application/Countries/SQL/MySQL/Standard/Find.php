...
class Find
{
    public static function execute($data)
    {
        ...
        $Qcountries->bind_value(':countries_name', '%' . $data['keywords'] . '%');
        $Qcountries->bind_value(':countries_iso_code_2', '%' . $data['keywords'] . '%');
        $Qcountries->bind_value(':countries_iso_code_3', '%' . $data['keywords'] . '%');
        $Qcountries->bind_value(':zone_name', '%' . $data['keywords'] . '%');
        $Qcountries->bind_value(':zone_code', '%' . $data['keywords'] . '%');
        if ($data['batch_pageset'] !== -1) {
