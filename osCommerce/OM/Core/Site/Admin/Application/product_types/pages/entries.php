...
echo osc_draw_button(['type' => 'button', 'params' => 'onclick="osC_DataTable.reset();"', 'title' => 'Reset']);
?></form></span>
  <span style="float: right;"><?php 
echo osc_draw_button(['href' => osc_href_link_admin(FILENAME_DEFAULT, $os_c_template->get_module()), 'icon' => 'triangle-1-w', 'title' => OSCOM::get_def('button_back')]) . ' ' . osc_draw_button(['href' => osc_href_link_admin(FILENAME_DEFAULT, $os_c_template->get_module() . '=' . $_GET[$os_c_template->get_module()] . '&action=entry_save'), 'icon' => 'plus', 'title' => OSCOM::get_def('button_insert')]);
?></span>
</div>
...
<table border="0" width="100%" cellspacing="0" cellpadding="2" class="dataTable" id="productTypesAssignmentsDataTable">
  <thead>
    <tr>
      <th><?php 
echo OSCOM::get_def('table_heading_actions');
?></th>
      <th><?php 
...
echo '<input type="image" src="' . osc_icon_raw('trash.png') . '" title="' . OSCOM::get_def('icon_trash') . '" onclick="document.batch.action=\'' . osc_href_link_admin(FILENAME_DEFAULT, $os_c_template->get_module() . '=' . $_GET[$os_c_template->get_module()] . '&action=batch_delete_entries') . '\';" />';
?></th>
      <th align="center" width="20"><?php 
echo osc_draw_checkbox_field('batchFlag', null, null, 'onclick="flagCheckboxes(this);"');
?></th>
...
echo osc_href_link_admin('rpc.php', $os_c_template->get_module() . '=' . (int) $_GET[$os_c_template->get_module()] . '&action=getAllAssignments');
?>';

  var entryEditLink = '<?php 
echo osc_href_link_admin(FILENAME_DEFAULT, $os_c_template->get_module() . '=' . (int) $_GET[$os_c_template->get_module()] . '&aID=ACTIONID&action=entry_save');
?>';
  var entryEditLinkIcon = '<?php 
echo osc_icon('edit.png');
?>';

  var entryDeleteLink = '<?php 
echo osc_href_link_admin(FILENAME_DEFAULT, $os_c_template->get_module() . '=' . (int) $_GET[$os_c_template->get_module()] . '&aID=ACTIONID&action=entry_delete');
?>';
  var entryDeleteLinkIcon = '<?php 
