...
class Find
{
    public static function execute($data)
    {
        ...
        $Qcustomers->bind_value(':customers_firstname', '%' . $data['keywords'] . '%');
        $Qcustomers->bind_value(':customers_lastname', '%' . $data['keywords'] . '%');
        if ($data['batch_pageset'] !== -1) {
            $Qcustomers->bind_int(':batch_pageset', $OSCOM_PDO->get_batch_from($data['batch_pageset'], $data['batch_max_results']));
            $Qcustomers->bind_int(':batch_max_results', $data['batch_max_results']);
        }
        $Qcustomers->execute();
        $result['entries'] = $Qcustomers->fetch_all();
        $Qcustomers->next_rowset();
        $result['total'] = $Qcustomers->fetch_column();
        return $result;
    }
}