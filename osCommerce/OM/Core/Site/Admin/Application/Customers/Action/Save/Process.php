 ...
 */
namespace Os_Commerce\OM\Core\Site\Admin\Application\Customers\Action\Save;
    ...
    public static function execute(Application_Abstract $application)
    {
        ...
        if (ACCOUNT_GENDER > 0) {
            if ($data['gender'] != 'm' && $data['gender'] != 'f') {
                $oscom_message_stack->add(null, OSCOM::get_def('ms_error_gender'), 'error');
                $error = true;
            }
        }
        if (strlen(trim($data['firstname'])) < ACCOUNT_FIRST_NAME) {
            $oscom_message_stack->add(null, sprintf(OSCOM::get_def('ms_error_first_name'), ACCOUNT_FIRST_NAME), 'error');
            $error = true;
        }
        if (strlen(trim($data['lastname'])) < ACCOUNT_LAST_NAME) {
            $oscom_message_stack->add(null, sprintf(OSCOM::get_def('ms_error_last_name'), ACCOUNT_LAST_NAME), 'error');
            $error = true;
        }
        if (ACCOUNT_DATE_OF_BIRTH == '1') {
            if (!checkdate($data['dob_month'], $data['dob_day'], $data['dob_year'])) {
                $oscom_message_stack->add(null, OSCOM::get_def('ms_error_date_of_birth'), 'error');
                $error = true;
            }
        }
        if (strlen(trim($data['email_address'])) < ACCOUNT_EMAIL_ADDRESS) {
            $oscom_message_stack->add(null, sprintf(OSCOM::get_def('ms_error_email_address'), ACCOUNT_EMAIL_ADDRESS), 'error');
            $error = true;
        } elseif (filter_var($data['email_address'], FILTER_VALIDATE_EMAIL) === false) {
            $oscom_message_stack->add(null, OSCOM::get_def('ms_error_email_address_invalid'), 'error');
            $error = true;
        } elseif (Customers::email_address_exists($data['email_address'], isset($_GET['id']) ? $_GET['id'] : null)) {
            $oscom_message_stack->add(null, OSCOM::get_def('ms_error_email_address_exists'), 'error');
            $error = true;
        }
        if ((!isset($_GET['id']) || !empty($data['password'])) && strlen(trim($data['password'])) < ACCOUNT_PASSWORD) {
            $oscom_message_stack->add(null, sprintf(OSCOM::get_def('ms_error_password'), ACCOUNT_PASSWORD), 'error');
            $error = true;
        } elseif (!empty($_POST['confirmation']) && trim($data['password']) != trim($_POST['confirmation'])) {
            $oscom_message_stack->add(null, OSCOM::get_def('ms_error_password_confirmation_invalid'), 'error');
            $error = true;
        }
        if ($error === false) {
        ...
        if ($error === false) {
            if (isset($_GET['id']) && is_numeric($_GET['id'])) {
                ...
                $customer_data = OSCOM::call_db('Admin\Customers\Get', ['email_address' => $data['email_address']]);
        ...
        if ($error === false) {
            if (isset($_POST['deleteAB']) && is_array($_POST['deleteAB']) && !empty($_POST['deleteAB'])) {
                foreach ($_POST['deleteAB'] as $ab_id) {
                    if (Customers::delete_address($ab_id, $customer_id) === false) {
                        $error = true;
                        break;
                    }
                }
        ...
        if ($error === false) {
            $oscom_message_stack->add(null, OSCOM::get_def('ms_success_action_performed'), 'success');
        } else {
            $oscom_message_stack->add(null, OSCOM::get_def('ms_error_action_not_performed'), 'error');
        }
