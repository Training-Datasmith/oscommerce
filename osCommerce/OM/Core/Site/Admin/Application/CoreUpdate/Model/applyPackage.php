...
class Apply_Package
{
    ...
    public static function execute()
    {
        ...
        try {
            ...
            if (isset($meta['delete'])) {
                foreach ($meta['delete'] as $file) {
                    $directory = (substr($file, 0, 14) == 'osCommerce/OM/' ? realpath(OSCOM::BASE_DIRECTORY . '../../') : realpath(OSCOM::get_config('dir_fs_public', 'OSCOM') . '../')) . '/';
                    if (file_exists($directory . $file)) {
                        if (is_dir($directory . $file)) {
                            if (rename($directory . $file, $directory . dirname($file) . '/.CU_' . basename($file))) {
                                $pro_hart[] = ['type' => 'directory', 'where' => $directory, 'path' => dirname($file) . '/.CU_' . basename($file), 'log' => true];
                            }
                        } else if (rename($directory . $file, $directory . dirname($file) . '/.CU_' . basename($file))) {
                            $pro_hart[] = ['type' => 'file', 'where' => $directory, 'path' => dirname($file) . '/.CU_' . basename($file), 'log' => true];
                        }
                    }
                }
            }
            ...
            foreach (new Recursive_Iterator_Iterator($phar) as $iteration) {
                if (($pos = strpos($iteration->get_path_name(), 'update.phar')) !== false) {
                    $file = substr($iteration->get_path_name(), $pos + 12);
                    $directory = (substr($file, 0, 14) == 'osCommerce/OM/' ? realpath(OSCOM::BASE_DIRECTORY . '../../') : realpath(OSCOM::get_config('dir_fs_public', 'OSCOM') . '../')) . '/';
                    if (file_exists($directory . $file)) {
                        if (rename($directory . $file, $directory . dirname($file) . '/.CU_' . basename($file))) {
                            $pro_hart[] = ['type' => 'file', 'where' => $directory, 'path' => dirname($file) . '/.CU_' . basename($file), 'log' => false];
                        }
                    }
                    if ($phar->extract_to($directory, $file, true)) {
            ...
            foreach (array_reverse($pro_hart, true) as $mess) {
                if ($mess['type'] == 'directory') {
                    ...
                    } else if ($mess['log'] === true) {
                        self::log('*** Could Not Delete: ' . str_replace('/.CU_', '/', $mess['path']));
                    }
                } else if (unlink($mess['where'] . $mess['path'])) {
                    if ($mess['log'] === true) {
                        self::log('Deleted: ' . str_replace('/.CU_', '/', $mess['path']));
                    }
                } else if ($mess['log'] === true) {
            ...
            foreach (array_reverse($pro_hart, true) as $mess) {
                ...
                } else if (file_exists($mess['where'] . str_replace('/.CU_', '/', $mess['path']))) {
        ...
        if ($phar_can_open === true) {
            if (isset($meta['run']) && method_exists('osCommerce\OM\Work\CoreUpdate\\' . $meta['run'] . '\Controller', 'runAfter')) {
                $results = call_user_func(['osCommerce\OM\Work\CoreUpdate\\' . $meta['run'] . '\Controller', 'runAfter']);
                if (!empty($results)) {
                    self::log('##### RAN AFTER');
    ...
    protected static function log($message)
    {
        if (is_writable(OSCOM::BASE_DIRECTORY . 'Work/Logs')) {
            file_put_contents(OSCOM::BASE_DIRECTORY . 'Work/Logs/update-' . self::$_to_version . '.txt', '[' . DateTime::get_now('d-M-Y H:i:s') . '] ' . $message . "\n", FILE_APPEND);
        }
    }
