...
echo osc_href_link_admin(FILENAME_DEFAULT, $os_c_template->get_module() . '&template=' . $module->get_code() . '&action=uninstall');
?>" method="post">
...
if ($module->get_code() == DEFAULT_TEMPLATE) {
  ...
  <p><?php 
    echo '<b>' . $os_c_language->get('uninstall_error_template_prohibited') . '</b>';
    ?></p>

  <p align="center"><?php 
    echo '<input type="button" value="' . $os_c_language->get('button_cancel') . '" onclick="document.location.href=\'' . osc_href_link_admin(FILENAME_DEFAULT, $os_c_template->get_module()) . '\';" class="operationButton" />';
    ?></p>
...
<?php 
  ...
  <p><?php 
    echo $os_c_language->get('introduction_uninstall_template');
    ?></p>
