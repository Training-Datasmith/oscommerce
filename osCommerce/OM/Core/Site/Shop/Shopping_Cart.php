...
class ShoppingCart
{
    ...
    public function __construct()
    {
        if (!isset($_SESSION['osC_ShoppingCart_data'])) {
            $_SESSION['osC_ShoppingCart_data'] = ['contents' => [],
                                                       'sub_total_cost' => 0,
                                                       'total_cost' => 0,
                                                       'total_weight' => 0,
                                                       'tax' => 0,
                                                       ...
                                                       'shipping_method' => [],
                                                       'billing_address' => ['zone_id' => STORE_ZONE,
                                                                                  'country_id' => STORE_COUNTRY],
                                                       'billing_method' => [],
                                                       'shipping_quotes' => [],
                                                       'order_totals' => []];
    ...
    public function synchronizeWithDatabase()
    {
        ...
        while ($Qproducts->fetch()) {
            if ($Qproducts->valueInt('products_status') === 1) {
                ...
                $this->_contents[$Qproducts->valueInt('item_id')] = ['item_id' => $Qproducts->valueInt('item_id'),
                    ...
                    if ($Qcheck->valueInt('products_status') === 1) {
                        ...
                        if (count($variants) > 0) {
                            foreach ($variants as $v) {
                                ...
                                $this->_contents[$Qproducts->valueInt('item_id')]['variants'][] = ['group_id' => $v['group_id'],
                                                                                                        'value_id' => $v['value_id'],
    ...
    public function add($product_id, $quantity = null)
    {
        ...
        if ($OSCOM_Product->isValid()) {
            if ($this->exists($product_id)) {
                ...
                $this->_contents[$item_id] = ['item_id' => $item_id,
                                                   'id' => $product_id,
                ...
                if ($OSCOM_Product->getData('parent_id') > 0) {
                    ...
                    while ($Qvariant->fetch()) {
                        ...
                        $this->_contents[$item_id]['variants'][] = ['group_id' => $Qvariant->valueInt('group_id'),
    ...
    public function setShippingAddress($address)
    {
        ...
        if ($OSCOM_Customer->isLoggedOn() && is_numeric($address)) {
            ...
            if ($Qaddress->fetch() !== false) {
                $this->_shipping_address = ['id' => (int)$address,
    ...
    public function resetShippingAddress()
    {
        ...
        $this->_shipping_address = ['zone_id' => STORE_ZONE,
                                         'country_id' => STORE_COUNTRY];
    ...
    public function setBillingAddress($address)
    {
        ...
        if ($OSCOM_Customer->isLoggedOn() && is_numeric($address)) {
            ...
            if ($Qaddress->fetch() !== false) {
                $this->_billing_address = ['id' => (int)$address,
                                                'firstname' => $Qaddress->valueProtected('entry_firstname'),
                                                'lastname' => $Qaddress->valueProtected('entry_lastname'),
                                                'gender' => $Qaddress->valueProtected('entry_gender'),
                                                'company' => $Qaddress->valueProtected('entry_company'),
    ...
    public function resetBillingAddress()
    {
        ...
        $this->_billing_address = ['zone_id' => STORE_ZONE,
                                        'country_id' => STORE_COUNTRY];
