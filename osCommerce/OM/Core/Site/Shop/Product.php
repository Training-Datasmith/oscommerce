...
class Product
{
            ...
            if (!empty($this->_data)) {
                ...
                if ((int)$this->_data['has_children'] === 1) {
                    ...
                    while ($Qsubproducts->fetch()) {
                        $this->_data['variants'][$Qsubproducts->valueInt('products_id')]['data'] = ['price' => $Qsubproducts->value('products_price'),
                        ...
                        while ($Qvariants->fetch()) {
                            $this->_data['variants'][$Qsubproducts->valueInt('products_id')]['values'][$Qvariants->valueInt('group_id')][$Qvariants->valueInt('value_id')] = ['value_id' => $Qvariants->valueInt('value_id'),
            ...
            foreach ($this->_data['type_assignments'][$action_call] as $module) {
                ...
                if (is_null($module_call) || ($module_call == $module)) {
                    if (class_exists('osCommerce\\OM\\Core\\Site\\Shop\\Module\\ProductType\\' . $module)) {
                        if (!call_user_func(['osCommerce\\OM\\Core\\Site\\Shop\\Module\\ProductType\\' . $module, 'isValid'], $this)) {
                            if (($execute_onfail === true) && method_exists('osCommerce\\OM\\Core\\Site\\Shop\\Module\\ProductType\\' . $module, 'onFail')) {
                                call_user_func(['osCommerce\\OM\\Core\\Site\\Shop\\Module\\ProductType\\' . $module, 'onFail'], $this);
                            }
                ...
                foreach ($variants['values'] as $group_id => $values) {
                    foreach ($values as $value_id => $value) {
                        ...
                        if ($value_exists === false) {
                            $values_array[$group_id]['data'][] = ['id' => $value_id,
    ...
    public function getAttribute($code)
    {
        ...
        if (class_exists('osC_ProductAttributes_' . $code)) {
            return call_user_func(['osC_ProductAttributes_' . $code, 'getValue'], $this->_data['attributes'][$code]);
        }
    }
