...
class Products
{
    ...
    public function setCategory($id, $recursive = true)
    {
        $this->_category = $id;
    ...
    public static function getListingSortLink($key, $heading)
    {
        ...
        return HTML::link(OSCOM::getLink(null, null, OSCOM::getAllGET(['page', 'sort']) . '&sort=' . $key . ($direction == '+' ? '|d' : '')), $heading . (($key == $current) ? $direction : ''), 'title="' . (isset($_GET['sort']) && ($_GET['sort'] == $key) ? sprintf(OSCOM::getDef('listing_sort_ascendingly'), $heading) : sprintf(OSCOM::getDef('listing_sort_descendingly'), $heading)) . '" class="productListing-heading"');
    }
