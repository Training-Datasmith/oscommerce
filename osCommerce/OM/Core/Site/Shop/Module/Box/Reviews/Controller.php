...
class Controller extends \osCommerce\OM\Core\Modules
{
    ...
    public function initialize()
    {
        ...
        if ($OSCOM_Service->isStarted('Reviews')) {
            ...
            if (empty($data)) {
                if (isset($OSCOM_Product) && ($OSCOM_Product instanceof \osCommerce\OM\Site\Shop\Product) && $OSCOM_Product->isValid()) {
                    $this->_content = '<div style="float: left; width: 55px;">' . HTML::button(['href' => OSCOM::getLink(null, 'Products', 'Reviews&Write&' . $OSCOM_Product->getKeyword()), 'icon' => 'pencil', 'title' => OSCOM::getDef('button_write_review')]) . '</div>' .
    ...
    {
        if (!isset($this->_keys)) {
            $this->_keys = ['BOX_REVIEWS_RANDOM_SELECT', 'BOX_REVIEWS_CACHE'];
        }
