...
class NavigationHistory
{
    ...
    public function addCurrentPage()
    {
        ...
        $this->_data[] = ['application' => OSCOM::getSiteApplication(),
                               'action' => $action_get,
    ...
    public function getPathURL($back = 1, $exclude = [])
    {
        if ((is_numeric($back) === false) || (is_numeric($back) && ($back < 1))) {
            $back = 1;
        }
    ...
    public function setSnapshot($page = null)
    {
        if (isset($page) && is_array($page)) {
            $this->_snapshot = ['application' => $page['application'],
                                     'action' => $page['action'],
                                     'mode' => $page['mode'],
                                     'get' => $page['get'],
                                     'post' => $page['post']];
    ...
    protected function parseParameters($array, $additional_exclude = [])
    {
        $exclude = ['x', 'y', Registry::get('Session')->getName()];
