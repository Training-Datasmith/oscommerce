...
class Http_Request
{
    protected static $_drivers = ['HttpRequest', 'Curl', 'Stream'];
    /**
     *
     * @param array $parameters url, header, parameters, method, certificate
     * @param string $driver
     */
    public static function get_response($parameters, $driver = null)
    {
        if (!isset($driver)) {
        ...
        return trim(call_user_func(['osCommerce\OM\Core\HttpRequest\\' . $driver, 'execute'], $parameters));
    }
}