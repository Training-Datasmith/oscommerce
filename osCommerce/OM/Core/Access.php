    ...
    public static function getUserLevels($id, $site = null)
    {
        ...
        if (in_array('*', $applications)) {
            ...
            foreach ($DLapps->getFiles() as $file) {
                if (preg_match('/[A-Z]/', substr($file['name'], 0, 1)) && !in_array($file['name'], call_user_func(['osCommerce\\OM\\Core\\Site\\' . $site . '\\Controller', 'getGuestApplications'])) && file_exists($DLapps->getDirectory() . '/' . $file['name'] . '/Controller.php')) { // HPDL remove preg_match
            ...
            foreach ($DLcapps->getFiles() as $file) {
                if (!in_array($file['name'], $applications) && !in_array($file['name'], call_user_func(['osCommerce\\OM\\Core\\Site\\' . $site . '\\Controller', 'getGuestApplications'])) && file_exists($DLcapps->getDirectory() . '/' . $file['name'] . '/Controller.php')) {
                    $applications[] = $file['name'];
                }
            }
        }
