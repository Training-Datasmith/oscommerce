...
class Language
{
    ...
    public function load($key, $language_code = null)
    {
        ...
        if ($this->_languages[$language_code]['parent_id'] > 0) {
            $this->load($key, $this->get_code_from_id($this->_languages[$language_code]['parent_id']));
        }
        $def_data = ['language_id' => $this->get_data('id', $language_code), 'group' => $key];
        foreach (OSCOM::call_db('GetLanguageDefinitions', $def_data, 'Core') as $def) {
    ...
    public function set($code = null)
    {
        ...
        if (!isset($_COOKIE['language']) || $_COOKIE['language'] != $this->_code) {
            OSCOM::set_cookie('language', $this->_code, time() + 60 * 60 * 24 * 90);
        }
    ...
    public function get_data($key, $language = null)
    {
        if (!isset($language)) {
            $language = $this->_code;
        }
    ...
    {
        if ($with_time === true) {
            return $this->_languages[$this->_code]['date_format_short'] . ' ' . $this->get_time_format();
        }
    ...
    public function show_image($code = null, $width = 16, $height = 10, $parameters = null)
    {
        ...
        return HTML::image(OSCOM::get_public_site_link('images/worldflags/' . $image_code . '.png', null, 'Shop'), $this->_languages[$code]['name'], $width, $height, $parameters);
    }
    /**
    ...
    public static function is_utf8($string)
    {
        if (strlen($string) > 5000) {
            for ($i = 0, $s = 5000, $j = ceil(strlen($string) / 5000); $i < $j; $i++, $s += 5000) {
                if (static::is_utf8(substr($string, $s, 5000))) {
                    return true;
                }
            }
            return false;
