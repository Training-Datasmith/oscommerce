...
class Mail
{
    ...
    public $_images = [];
    public $_boundary;
    public $_headers = ['X-Mailer' => 'osCommerce'];
    public $_body;
    public $_content_transfer_encoding = '7bit';
    ...
    public function __construct($to = null, $to_email_address = null, $from = null, $from_email_address = null, $subject = null)
    {
        if (!empty($to_email_address)) {
            $this->_to[] = ['name' => $to, 'email_address' => $to_email_address];
        }
        if (!empty($from_email_address)) {
            $this->_from = ['name' => $from, 'email_address' => $from_email_address];
        }
        if (!empty($subject)) {
            $this->_subject = $subject;
        }
    }
    public function add_to($name = null, $email_address)
    {
        $this->_to[] = ['name' => $name, 'email_address' => $email_address];
    }
    public function set_from($name = null, $email_address)
    {
        $this->_from = ['name' => $name, 'email_address' => $email_address];
    }
    public function add_cc($name = null, $email_address)
    {
        $this->_cc[] = ['name' => $name, 'email_address' => $email_address];
    }
    public function add_bcc($name = null, $email_address)
    {
        $this->_bcc[] = ['name' => $name, 'email_address' => $email_address];
    }
    public function clear_to()
    {
        ...
        $this->_cc = [];
        $this->_bcc = [];
        $this->_headers = ['X-Mailer' => 'osCommerce'];
    }
    public function set_subject($subject)
    {
        $this->_subject = $subject;
    }
    public function set_body_plain($body)
    {
    ...
    public function set_content_transfer_encoding($encoding)
    {
    ...
    public function add_attachment($file, $is_uploaded = false)
    {
        if ($is_uploaded === true) {
        ...
        $this->_attachments[] = ['filename' => $filename, 'mimetype' => $mimetype, 'data' => chunk_split(base64_encode($data))];
    }
    public function add_image($file, $is_uploaded = false)
    {
        if ($is_uploaded === true) {
        } elseif (file_exists($file) && is_readable($file)) {
            $data = file_get_contents($file);
        ...
        $this->_images[] = ['id' => md5(uniqid(time())), 'filename' => $filename, 'mimetype' => $mimetype, 'data' => chunk_split(base64_encode($data))];
    }
    public function send()
    {
        ...
        if (empty($this->_from['name'])) {
            $this->add_header('From', $this->_from['email_address']);
        } else {
            $this->add_header('From', '"' . $this->_from['name'] . '" <' . $this->_from['email_address'] . '>');
        }
    ...
    public function _get_mime_type($file)
    {
    ...
    public function _build_attachment($attachment, $boundary)
    {
        return '--' . $boundary . "\n" . 'Content-Type: ' . $attachment['mimetype'] . '; name="' . $attachment['filename'] . '"' . "\n" . 'Content-Disposition: attachment' . "\n" . 'Content-Transfer-Encoding: base64' . "\n\n" . $attachment['data'] . "\n\n";
    }
    public function _build_image($image, $boundary)
    {
